...
struct EditOTPPageFallback: View { /// This insanely dumb workaround (duplicated view) prevents a crash on iOS 14 when an attribute is marked with `@available(iOS 15, *) @FocusState`
...
struct EditOTPPageFallbackPreview: PreviewProvider {
    
    static var previews: some View {
        PreviewDevice.generate {
            NavigationView {
                EditOTPPageFallback(otp: OTP.mock, updateOtp: { _ in })
            }
            .showColumns(false)
        }
    }
